...
1. **同级比较，跳过跨层级对比**，React **只会比较同级元素**，如果发现节点类型不同，就会直接删除原节点，创建新节点，而不会尝试**跨层级对比**。

2. **相同类型的元素，比较属性**，当 React 发现 **节点类型相同**，只会**对比属性（Props）**，然后**只更新变化的属性**，保持 DOM 结构不变。

3. **子元素 Diff 三种情况**
   当 **子元素顺序未改变** 时，React 只需逐个对比。
   如果新列表 **增加或删除节点**，React 会**销毁旧节点，创建新节点**。

   当元素 **顺序发生变化** 时，React 可能会误以为某些节点是新的，从而销毁旧的节点，重新创建。**引入 key 可以优化这个问题**。
